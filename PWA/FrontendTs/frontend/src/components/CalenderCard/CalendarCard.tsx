...
export default function CalendarCard(props: ICalendarProps) {
    function getReservations(startDate: Date) {
        const date_reservations = props.car.reservations.filter((x) => x.startDate === startDate);
                ...
                return (
                    <div className="calendar-date">
                        <div className="date-title">
                            <h2 className="date">{moment(reservation.startDate).format("LT")}</h2>
                        </div>
                            ...
                            return (
                                <div className="calendar-person">
                                    ...
                                    <h1 className="calendar-name">
                                        {moment(reservation.startDate).format("LT")}-
                                        {moment(reservation.endDate).format("LT")}
                                    </h1>
                                </div>
                            );
                        })}
